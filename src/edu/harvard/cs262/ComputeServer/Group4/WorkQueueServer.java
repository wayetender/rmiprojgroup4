...
public class WorkQueueServer implements WorkQueue, ComputeServer {
	...
	/**
	 * Registers a new worker, and notifies the Object monitor of a new free
	 * worker.
	 */
	@Override
	public synchronized UUID registerWorker(ComputeServer server)
	 ...
	 */
	@Override
	public synchronized boolean unregisterWorker(UUID workerID)
			throws RemoteException {
	...
	/**
	 * Waits until a worker is free to work on a task. Side effect: changes
	 * worker status from free to busy. Must call returnFreeWorker when the
	 * worker should be considered free for new tasks.
	 * 
	...
	private synchronized UUID getFreeWorker() {
		...
		while (firstFreeID == null) {
			try {
				System.out
						.println("no worker available, waiting for one to become available");
				wait();
	...
	/**
	 * Returns a worker to the free pool and removes them from being busy.
	 * Notifies the Object monitor of the newly free worker
	 * 
	 * @param freeWorker
	 ...
	 */
	private Object delegateWork(UUID worker, WorkTask work)
			throws RemoteException {
	...
	/**
	 * Sends work to a worker to perform. If no workers are available, it waits
	 * until a worker becomes free. If there was a ConnectionException while
	 * delegating the work, it is unregistered and the work task is performed on
	...
	public Object sendWork(WorkTask work) throws RemoteException {
		while (true) {
			...
			try {
				...
				System.out.println("problem delegating task to worker "
						+ firstFreeID + "; removing");
	...
	/**
	 * Creates a WorkQueueServer and exports it as a remote and registers it
	 * with the locate registry.
	 * 
	 * @return the newly created WorkQueueServer.
	 * @throws Exception
	 *             problem while exporting the object or registering it with the
	 *             locate registry.
	 */
